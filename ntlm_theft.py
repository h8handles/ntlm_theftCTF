...
parser.add_argument('-u', '--upload',action='store_true',help="Optional arg to upload to the victim share", dest='upload',required=False)
parser.add_argument('-rserver', help='The address of the SMB server to upload to')
parser.add_argument('-rport', help='The port of the SMB server to upload to',type=int,default=445)
parser.add_argument('-smbuser', help='The username to use to authenticate to the SMB server',default="Guest")
parser.add_argument('-smbpass', help='The password to use to authenticate to the SMB server',default="")
parser.add_argument('-d', help="The directory we upload files from",required=True)



args = parser.parse_args()

#arguments to set the remote server to upload to
SMBUSER = args.smbuser
SMBPASS = args.smbpass
SMBSERVER = args.rserver
SMBPORT = args.rport

#establish connection to the smb server and return the connection
def connect_to_smb_server():
    conn = SMBConnection(SMBUSER, SMBPASS, "my_client", SMBSERVER, use_ntlm_v2=True)
    connected = conn.connect(SMBSERVER, SMBPORT, timeout=10)
    if not connected:
        print("Failed to connect to the SMB server.")
        return None
    return conn

#create the function to upload all the generated files
def upload_filesconnect_to_smb_server(rserver, port, username, password, directory):
	conn = SMBConnection(username, password, "localhost", rserver, use_ntlm_v2=True)
	if conn.connect(rserver, port, timeout=30):
...
def upload_files(conn):
	if conn is None:
		...
		return
	print(f"Uploading from directory: {args.d}")
	for root, dirs, files in os.walk(args.d):
		for filename in files:
			if not filename.endswith(".py"):
				file_path = os.path.join(root, filename)
				print(f"Attempting to upload: {file_path}")  # Debugging line
			try:
...
# =   # Replace "/path/to/generated/files" with the actual directory path
'''
this doesn't work 
it will try to upload from the directory but also to the same named directory on the smb server
it needs to upload to /share not ./desktop/filename

'''
print("Generation Complete.")

if args.upload:
	conn = connect_to_smb_server()
	if conn:
		upload_files(conn)
